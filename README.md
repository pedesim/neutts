...
>
> **Pi 5 setup tip:** I've had the best results using the Q4 GGUF model with 4 threads (`--threads 4`). The Q8 model works too but runs noticeably warmer and slower under sustained load.
>
> **Audio output tip:** On Pi 5 with the default ALSA setup, I pipe output through `aplay -r 22050 -f S16_LE -c 1` — saves fiddling with sample rate mismatches. Also worth setting `--chunk-size 512` if you notice crackling at the start of utterances.
